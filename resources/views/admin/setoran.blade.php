...
<nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" data-scroll="true">
    <div class="container-fluid py-1 px-3">
        ...
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Setoran</li>
        </ol>
        <h6 class="font-weight-bolder mb-0">Setoran</h6>
        </nav>
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-info shadow-info border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3 text-center">Setoran</h6>
              </div>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('store.setoran', ['id_siswa' => $data->id_siswa]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('POST')
                    ...
                    <div class="my-1">
                        ...
                        <input type="number" class="form-control bg-light rounded border px-3" name="jumlah_setoran">
                    </div>
                    @if(@$errors->has('jumlah_setoran'))
                    <div class="alert alert-danger alert-dismissible text-white my-2">
                        <div class="text-sm d-flex justify-content-start align-items-center">
                            <i class="material-icons me-2" style="font-size: 26px;">cancel</i>
                            <strong class="me-1">Jumlah setoran tidak boleh kosong!</strong>
                        </div>
                    <button type="button" class="btn-close text-lg py-3 opacity-10" data-bs-dismiss="alert" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <button type="submit"class="btn bg-gradient-success mt-3 px-5" onclick="return confirm('Setor tabungan?')">Setor</button>
                    </div>
                </form>
            </div>
          </div>
